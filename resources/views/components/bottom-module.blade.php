...
<div class="w-1/3 relative float-left top-full {{$class}}" class="" x-data="{open: false}" :class='open ?  "z-50" : "z-30"'>
  ...
  <div class="fixed md:hidden w-screen  top-[0.85rem] right-4 text-right duration-200 transition-opacity" :class='open ?  "block opacity-100" : " hidden opacity-0"'> x close</div>
  ...
  <div @click.away="open=false">
    <div class=" relative group max-h-[calc(var(--doc-height)+6px)] bg-white border-x-2 border-t-2 border-black transition transform" :class='open ?  "translate-y-[calc(-100%-4px)]" : " -translate-y-[3.5rem] "'>
      ...
      <div class="border-t-2 w-screen md:w-auto bg-white text-left border-black  relative {{$slotClass}}  md:left-0  max-h-[calc(var(--doc-height)-3.75rem)] overflow-y-scroll" :class='open ?  "z-40" : "z-30"'>
