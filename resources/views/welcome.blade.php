...
<div x-data="artwork">
    ...
    <div id="artworkDetail" class="fixed z-50 gap-12   w-screen h-full top-0 left-0" :class="openDetail ? 'block' : 'hidden'">
        <p class="text-center relative z-50 mt-12" @click="imageClicked()">
            x close
        </p>
        <div id="artworkDetailBG" class="bg-white/95 z-0 absolute w-full h-full top-0 left-0"></div>

        <div class="flex flex-wrap md:flex-nowrap z-50 h-[calc(100%-4.5rem)] absolute w-full relaltive gap-12">
            <div class="md:w-2/3  w-full" id="artworkDetailImageWrapper">

            </div>
            <div class="md:w-1/3 w-full pr-12 py-12">
                <h1 class="slideUp opacity-0 text-3xl" id="artworkDetailtitle" x-text="artworkTitle">
                    Dit is een titel van een artwork
                </h1>
                <div class="flex gap-2 mt-4" id="artworkDetailCategories" >
                    <template x-for="category in artworkCategories">
                        <p class="bg-black opacity-0 slideUp opacity-0 text-white px-2 py-1 uppercase text-xs" x-text='category'></p>
                    </template>
                </div>
                <div class="mt-8 slideUp opacity-0">
    ...
    <div id="slider_wrap" class=" h-[22rem] sm:h-[34rem] w-screen z-20 fixed top-[calc(50vh-2rem)] left-1/2 transform -translate-x-1/2 -translate-y-1/2 ">
        <div id="slider" @scroll.window="scrollArtworks()" class="noscrollbar   absolute top-0 left-0  h-full items-start flex ">
            @foreach($artworks as $artwork)
            <div @click="imageClicked({{$artwork->id}})" class="imageCarouselItem {{$artwork->getCategoriesString()}}  self-start  px-8 h-full group  hover:scale-110  scale-100 transition-transform duration-500 py-20 sm:py-32 transform overflow-hidden origin-center">
